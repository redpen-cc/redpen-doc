...
greater detail. The settings for the symbols section are described in
setting-characters-section.
...
We will cover all supported validators in the validator page.
...
and "ja", which are described in en-default-symbol-setting and
ja-default-symbol-setting.
...
provide the two defalut symbol settings for Japanese. The valiations are
